...
const DoctorReg = ({ state }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (DoctorData.status === 200 && HospitalData.status === 400) {
      ...
      } else if (DoctorData.status === 400) {
        setLoading(false);
        setToastMessage("Doctor License Id already Exists.");
        setShowToast(true);
      } else if (HospitalData.status === 200) {
        setLoading(false);
        setToastMessage(
          "Hospital Id not exists. Please use a Currect Hospital Id."
        );
        setShowToast(true);
      } else {
    ...
    <div className="min-h-screen bg-gray-100 text-gray-900 flex justify-center">
      <div className="max-w-screen-xl m-0 sm:m-10 bg-white shadow sm:rounded-lg flex justify-center flex-1 border border-gray-300">
        <div
          ...
          style={{
            backgroundImage:
              'url("https://images.unsplash.com/photo-1584467735871-8e85353a8413?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")',
          }}
        ></div>
        <div className="flex items-center w-full max-w-3xl p-8 mx-auto lg:px-12 lg:w-3/5">
          <div className="w-full">
            ...
            <p className="mt-4 text-gray-500">
              Let's get you all set up so you can verify your Doctor Account and
