  ...
  useEffect(() => {
    const fetchHospitalDetails = async () => {
      try {
        ...
        if (data.status === 200) {
          const hospitalDetails = data.HospitalDetails;
          setHospitalName(hospitalDetails.hospitalName);
          setContactNumber(hospitalDetails.numcontactNumber.toString());
          setHospitalAddress(hospitalDetails.hospitalAddress);
          setHospitalSpecialization(hospitalDetails.hospitalSpec);
        }
      } catch (error) {
      ...
      if (data.status === 400) {
        if (contract && contract.methods) {
          await contract.methods
            ...
            .send({ from: account });
          clearForm();
          setPopupMessage("Hospital Details Update Successful");
          setPopupColor("blue");
          setShowPopup(true);
          setIsLoading(false);
        }
      } else {
        setPopupMessage("Hospital Details cannot be Update");
        setPopupColor("red");
        setShowPopup(true);
        setIsLoading(false);
      }
      ...
      <div className="pt-8">
        <div className="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl border border-gray-300">
          <div
            ...
            style={{
              backgroundImage:
                'url("https://images.unsplash.com/photo-1524683745036-b46f52b8505a?q=80&w=1915&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")',
            }}
          />
