...
const HospitalLogin = ({ state }) => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (data.status === 200) {
        const existingHospitalID = Cookies.get("hospitalId");
        if (existingHospitalID) {
          Cookies.remove("hospitalId");
        }
        Cookies.set("hospitalId", hospitalId);
        navigate(`/admin/hospital-view/${hospitalId}`);
      } else {
        setShowPopup(true);
        ShowPopupMessage("Invalid Hospital Id and Password");
      }
    } catch (error) {
      ...
      <div className="py-6">
        <div className="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl border border-gray-300">
          <div
            ...
            style={{
              backgroundImage:
                'url("https://plus.unsplash.com/premium_photo-1675686363462-cc90eed33b0d?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")',
            }}
          />
